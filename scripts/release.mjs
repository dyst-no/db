...
const getCommitMessages = (lastTag) => {
  ...
  const output = capture(`git log ${range} --reverse --pretty=format:%H%x09%h%x09%s`);
  ...
  return output
    ...
    .map((line) => {
      ...
      const commitRef = commitUrlBase ? `[\`${shortHash}\`](${commitUrlBase}/commit/${hash})` : `\`${shortHash}\``;
...
const prependChangelog = (version, commitMessages) => {
  ...
  const body = existing.startsWith(header) ? existing.slice(header.length).trimStart() : `\n\n${existing}`;
...
const ensureCleanWorktree = () => {
  ...
  if (status) {
    throw new Error('Release requires a clean git working tree. Commit or stash changes first.');
  }
};
